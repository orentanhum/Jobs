...
type Profile = {
  ...
  professional_summary?: string; target_roles?: string[]; preferred_locations?: string[]; work_modes?: string[]; keywords?: string[];
  preferred_companies?: string[]; excluded_companies?: string[]; excluded_keywords?: string[]; minimum_fit?: number; auto_discovery?: boolean;
  cv_file_name?: string; cv_storage_path?: string; cv_updated_at?: string
}
...
export default function SettingsPage() {
  ...
  const save = async () => {
    ...
    const editable = {
      ...
      professional_summary:p.professional_summary??'', target_roles:p.target_roles??[], preferred_locations:p.preferred_locations??[], work_modes:p.work_modes??[],
  ...
  const analyze = async () => {
    setMsg('Analyzing CV… Hebrew content will be converted to English capabilities.')
    const { data, error } = await supabase.functions.invoke('analyze-cv')
    if (error) throw error
    if (!data?.ok) throw new Error(data?.error || 'Analysis failed')
    setP(x => ({ ...x, keywords:data.skills || x.keywords }))
    setMsg(`CV analyzed successfully. ${data.skills_count || 0} capabilities saved in English.`)
    return data
  }
  ...
  const upload = async (f?: File) => {
    ...
    try {
      ...
      setMsg(`CV uploaded successfully: ${f.name}. Starting analysis…`)
      await analyze()
